...
const ServicesReport: React.FC = () => {
  ...
  useEffect(() => {
    const processData = () => {
      ...
      const serviceCounts: Record<number, {
        count: number;
        revenue: number;
      }> = {};
  ...
  // Calcular métricas de resumen
  const calculateSummaryMetrics = (data: ServicePopularity[], totalRevenue: number) => {
    ...
    const totalServicesCount = data.reduce((sum, service) => sum + service.count, 0);
