...
export const useGroups = () => {
  ...
  const fetchGroups = useCallback(async (): Promise<Group[]> => {
    ...
    try {
      ...
      // Procesar los datos para asegurar que user_count exista
      const processedGroups = response.data.map((group) => ({
        ...group,
        // Si user_count no está definido pero user_set sí, calcular el conteo
        user_count:
          group.user_count ?? (group.user_set ? group.user_set.length : 0),
      }));
  ...
  const createGroup = useCallback(
    async (groupData: GroupFormData): Promise<Group> => {
      setLoading(true);
      setError(null);
  ...
  const updateGroup = useCallback(
    async (id: number, groupData: GroupFormData): Promise<Group> => {
      setLoading(true);
      setError(null);
  ...
  const updateGroupPermissions = useCallback(
    async (id: number, permissionIds: number[]): Promise<boolean> => {
      setLoading(true);
      setError(null);
      ...
      try {
        const axiosInstance = createAxiosInstance();
        await axiosInstance.put(`${API_URL}${id}/permissions/`, {
